...
exports.signInWithLine = onCall(async (request) => {
  ...
  try {
    ...
    // หากต้องการความปลอดภัยสูงสุด ควรยิงไปที่
    ...
    try {
      ...
      console.log(
          `Found existing user by email: ${lineEmail}, UID: ${firebaseUid}`,
      );
      ...
      if (error.code === "auth/user-not-found") {
        ...
        console.log(
            `User with email ${lineEmail} not found. Creating new user.`,
        );
    ...
    throw new Error(
        `An error occurred while processing the LINE login: ${error.message}`,
    );
  }
});
